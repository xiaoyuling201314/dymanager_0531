$(function(){
	...
	$("#btnSave").on("click",function(){
		...
		if(validate(repairOrderNumber,instrumentFuselage,receivedDate,planCompleteDate,repairMan,faultDescription)){
		     ...
		     var addType="special";
		     var repairObject={"addType":addType,"sapNo":sapNo,"repairOrderNumber":repairOrderNumber,"receivedDate":receivedDate,"planCompleteDate":planCompleteDate,"repairMan":repairMan,"faultDescription":faultDescription,"faultPicture":faultPicture,"processingMethod":processingMethod,"processingPicture":processingPicture,"comments":comments,"actualCompleteDate":actualCompleteDate,"instrumentFuselage":instrumentFuselage};//"shipmentNo":shipmentNo,
			$.ajax({  
 	...
 	//导入维修记录
	 	$("#btnShipSave").on(
			"click",
			function() {
				$("#btnShipSave").attr("disabled","disabled");
				$("#btnShipSave").text("上传中...");
