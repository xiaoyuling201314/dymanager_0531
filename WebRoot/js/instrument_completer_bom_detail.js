...
$(function(){
  	//设置列表可编辑
	 $("#completerForm .edit").dblclick(function(){
		    var td = $(this);
		    ...
		    txt.blur(function(){
		        ...
		        var quantityNum="";
		         if($(td).attr("id")=='quantity'){
		           ...
		           $.ajax({
		             	url:"completeService/updateDetail.do",
						type : "POST",
						data : {
							"quantityNum" : newText,"completerId":id,"comment":comment
						},
						dataType:"json",
		...
		var completeId = $("input[name=completeId]").val();
		if (materNoList != '') {
			$.ajax({
				url : "completeService/addDetail.do",
				type : "POST",
				dataType : "json",
				data : {
					"completeId" : completeId,
					"materNoList" : materNoList
				},
				success : function(data) {
					...
					$("#completerForm tr").empty("");
					location.href=basePath+"completeService/selectComplete.do?id="+completeId;
				}
			})
		}
	});
